...
export class GeminiService {
  ...
  async generateStructuredPrompt(videoMetadata, userPrompt) {
    ...
    let data;
    try {
      console.log(`[GeminiService] Sending prompt request with model: ${this.modelName}`);
      data = await makeRequest(this.modelName);
    } catch (err) {
      if (this.modelName === 'gemini-2.5-flash') {
        console.warn(`[GeminiService] ${this.modelName} prompt request failed: ${err.message}. Retrying with gemini-1.5-flash fallback...`);

...
export class GeminiService {
  ...
  async generateStructuredPrompt(videoMetadata, userPrompt) {
    ...
    while (attempt < maxRetries) {
      try {
        console.log(`[GeminiService] Sending prompt request with model: ${this.modelName} (Attempt ${attempt + 1}/${maxRetries})`);
        ...
        if (isTransient && attempt < maxRetries) {
          ...
          console.warn(`[GeminiService] Transient error (503/429) on attempt ${attempt}: ${err.message}. Retrying in ${Math.round(delay)}ms...`);
          await new Promise(r => setTimeout(r, delay));
        } else {
          throw new Error(`Gemini API request failed after ${attempt} attempts: ${err.message}`);
        }
      }
    }
